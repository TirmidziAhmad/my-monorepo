          ...
          {techstackData.map((tech, index) => (
            <motion.div
              ...
              className={`group relative flex flex-col items-center justify-center p-6 rounded-2xl 
                ...
                hover:shadow-[0_0_30px_-5px_rgba(255,255,255,0.3)]
              `}
            >
              {/* Glow Background on Hover */}
              <div className="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-10 transition-opacity duration-300 bg-white" />

              <div className="text-5xl mb-4 z-10 transition-colors duration-300 text-gray-400 group-hover:text-white">
                {tech.icon}
              </div>
              <h3 className="text-sm md:text-base font-semibold text-gray-500 group-hover:text-white z-10 tracking-wider uppercase transition-colors duration-300">
                {tech.name}
              </h3>
            </motion.div>
          ))}
